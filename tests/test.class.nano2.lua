local class = require "mini.class.nano2"
...
local foo1 = assert( instance(foo) )
assert(foo1:hello()=="hello")

local bar1 = assert( instance(bar) )
...
local echo1 = assert( instance(echo_class, "la la la la") )
local echo2 = assert( instance(echo_class, "pom pom pom") )
