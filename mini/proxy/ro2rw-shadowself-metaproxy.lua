...
local function ro2rwss(orig, map)
	...
	map = map and G.setmetatable(map, {__index=defaultmap}) or defaultmap -- FIXME: copy the map and/or make a generator to keep the map unchangable
...
--			--	ignore deleted key, if value exist return the __index(_self, k) value ?
			local _next = function(t0, k0)
				...
				if t0==pubenv then
					local k1,v1 = G.next(orig, k0)
					local v2 = _t[k1] -- will use __index and return the proxy ?
					print("warning: v1 != v2", v1, v2)
 					return k1,v2
				else
					return G.next(t0, k0)
				end
...
local M = setmetatable({
	ro2rwss=ro2rwss,
	mkproxy1 = mkproxies.mkproxy1,
	mkproxy2 = mkproxies.mkproxy2,
	mkproxy1prefix = mkproxies.mkproxy1prefix,
	mkproxy2prefix = mkproxies.mkproxy2prefix,
