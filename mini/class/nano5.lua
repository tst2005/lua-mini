...
local base = {
	extend = function(self, subtype)
		...
		class_mt.__call = function(self, ...)
			...
			if class_mt.__metainit then
				class_mt.__metainit(instance, meta)
				class_mt.__metainit = nil
			end
			return assert(instance:init(...), "instance is nil ? check the class:init() function return")
		end
		class_mt.__metainit = subtype.metainit
...
local function common_class(name, prototype, parent)
	...
	klass.metainit = (prototype or {}).metainit or (parent or {}).metainit
	assert(klass.init)
